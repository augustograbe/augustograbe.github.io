...
function mainEntrance() {
    ...
    var runanimation = (() => {
        ...
        // Código do trabalho. A função pega as posições de cada aresta do quadrado e confere se está passando ou encostando nas bordas do canvas.
        // Caso alguma aresta encoste no canvas o quadrado irá girar no sentido contrário. (mudando o valor de incrment para positivo ou negativo)
        function canvasColision(){
            let edges_positions = [];
            for(let i =0;i<4; i++){
                edges_positions.push(new Vector4([...getVertex(cornersIndexes[i]), 0.0, 1.0]));
                edges_positions[i]= modelMatrix.multiplyVector4(edges_positions[i]);
                let edge = edges_positions[i].elements;
                let translateMatrix = new Matrix4();
                if (edge[0] < -wsize / 2 )
                {
                    translateMatrix.setTranslate(0.08, 0, 0);
                    modelMatrix = translateMatrix.multiply(modelMatrix);
                    increment*=-1;
                }
                else if(edge[0] > wsize / 2){
                    translateMatrix.setTranslate(-0.08, 0, 0);
                    modelMatrix = translateMatrix.multiply(modelMatrix);
                    increment*=-1;
                }
                else if(edge[1] < -wsize / 2){
                    translateMatrix.setTranslate(0, 0.08, 0);
                    modelMatrix = translateMatrix.multiply(modelMatrix);
                    increment*=-1;
                }
                else if( edge[1] > wsize / 2)
         ...
         */
        return () => {
            canvasColision(); //Confere trata todo frame caso o quadrado encoste na borda do canvas
            ang += increment;
            ang = ang % 360;
            if (click) {
